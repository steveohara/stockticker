...
Dim Img     As Long
    ...
    asByte = StrConv(Pic, vbFromUnicode)
    If CreateStreamOnHGlobal(asByte(0), 0, objImageStream) = 0 Then
        If GdipLoadImageFromStream(objImageStream, Img) <> 0 Then
            Err.Raise 999, "GDI+ Module", "Error loading picture from String"
            Exit Function
        Else
            ...
            ' Create the picture
            Set LoadPictureFromStringGDIPlus = CreatePicture(hBitmap)
